    ...
    class Document
    {
      ...
      public:
        ...
        Document (std::initializer_list<std::pair<std::string, Element>> list)
        {
            for (auto i : list)
            {
                m_data.emplace (i.first, i.second);
                m_field_names.emplace (i.first);
            }
        }
        ...
        {
            m_data.at (name) = Element (data, ti);
        }
         ...
         */
        void combine (const bson::Document d)
        {
            m_data.insert (d.begin(), d.end());
            m_field_names.insert (d.m_field_names.begin(), d.m_field_names.end());
            return;
        }
